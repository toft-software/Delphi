...
implementation
uses clsGenHlp, clsObjModel, clsDBHlp, frmSystemBesked, frmRedigervmDiscounts;

{$R *.dfm}

procedure TFormAllVMRabatter.actOpretExecute(Sender: TObject);
var
  tmpProduct : TFormRedigervmDiscounts;
begin
  tmpProduct := TFormRedigervmDiscounts.Create(self);
  tmpProduct.SetupAndShow(0);
  tmpProduct.Free;
//  UpdateCtrl
end;

procedure TFormAllVMRabatter.actRedigerExecute(Sender: TObject);
var
  tmpvmDiscounts : TFormRedigervmDiscounts;
begin
  tmpvmDiscounts := TFormRedigervmDiscounts.Create(self);
  tmpvmDiscounts.SetupAndShow(self.pvmDiscountsListe.ClientDataSet.FieldByName('Id').AsInteger);
  tmpvmDiscounts.Free;
 // UpdateCtrl;
