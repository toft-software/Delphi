...
uses
  ...
  LMDRTFActions, Vcl.ImgList, cxCurrencyEdit;
...
function TFormRedigerProduct.CheckData: Boolean;
Var
  tmpRes: String;
...
begin
  result:=true;
  tmpRes:='';
  if (self.edVarenr.Text = '') then
  Begin
    tmpRes:=tmpRes+'Der skal angives et navn på varen !'+chr(13)+chr(10);
    result:=false;
  End;
  if not result then
  Begin
...
begin
  ...
  self.pProduct.price := edPris.Value;
...
begin
  ...
  edPris.Value := self.pProduct.price;
