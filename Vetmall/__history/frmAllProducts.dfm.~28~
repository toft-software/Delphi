object FormAllProducts: TFormAllProducts
  Left = 0
  Top = 0
  Caption = 'VetMall Klient - Vareoversigt'
  ClientHeight = 607
  ClientWidth = 861
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  DesignSize = (
    861
    607)
  PixelsPerInch = 96
  TextHeight = 13
  object cxGroupBox1: TcxGroupBox
    Left = 16
    Top = 8
    Anchors = [akLeft, akTop, akRight, akBottom]
    Caption = 'Varer'
    ParentFont = False
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -13
    Style.Font.Name = 'Ariel'
    Style.Font.Style = []
    Style.IsFontAssigned = True
    TabOrder = 0
    DesignSize = (
      813
      549)
    Height = 549
    Width = 813
    object edVarenr: TcxTextEdit
      Left = 11
      Top = 22
      ParentFont = False
      Properties.OnChange = edVarenrPropertiesChange
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Arial'
      Style.Font.Style = []
      Style.IsFontAssigned = True
      TabOrder = 0
      Width = 130
    end
    object grVarer: TcxGrid
      Left = 12
      Top = 52
      Width = 791
      Height = 474
      Anchors = [akLeft, akTop, akRight, akBottom]
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Ariel'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      object grVarerDBTableView1: TcxGridDBTableView
        Navigator.Buttons.CustomButtons = <>
        DataController.DataSource = dsproducts
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        OptionsCustomize.ColumnsQuickCustomization = True
        OptionsData.Deleting = False
        OptionsData.Editing = False
        OptionsData.Inserting = False
        OptionsSelection.CellSelect = False
        OptionsView.GroupByBox = False
        object grVarerDBTableView1varenr: TcxGridDBColumn
          Caption = 'Varenr.'
          DataBinding.FieldName = 'model'
          Width = 116
        end
        object grVarerDBTableView1varenavn: TcxGridDBColumn
          Caption = 'Varenavn'
          DataBinding.FieldName = 'varenavn'
          Options.Editing = False
          Width = 190
        end
        object grVarerDBTableView1styrke: TcxGridDBColumn
          Caption = 'Styrke'
          DataBinding.FieldName = 'vmstrength'
          Options.Editing = False
          Width = 162
        end
        object grVarerDBTableView1form: TcxGridDBColumn
          Caption = 'Form'
          DataBinding.FieldName = 'vmform'
          Options.Editing = False
          Width = 216
        end
        object grVarerDBTableView1pakning: TcxGridDBColumn
          Caption = 'Pakning'
          DataBinding.FieldName = 'vmpackage'
          Options.Editing = False
          Width = 125
        end
        object grVarerDBTableView1Column1: TcxGridDBColumn
          DataBinding.FieldName = 'product_id'
        end
      end
      object grVarerLevel1: TcxGridLevel
        GridView = grVarerDBTableView1
      end
    end
    object btnNulstil: TcxButton
      Left = 778
      Top = 22
      Width = 25
      Height = 24
      Action = actRens
      Anchors = [akTop, akRight]
      LookAndFeel.NativeStyle = True
      OptionsImage.Glyph.Data = {
        
        F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Ariel'
      Font.Style = []
      ParentFont = False
    end
    object cbCategoryGroup: TcxComboBox
      Left = 480
      Top = 22
      Properties.OnChange = cbCategoryGroupPropertiesChange
      TabOrder = 3
      Width = 281
    end
    object edVarenavn: TcxTextEdit
      Left = 143
      Top = 22
      ParentFont = False
      Properties.OnChange = edVarenavnPropertiesChange
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Arial'
      Style.Font.Style = []
      Style.IsFontAssigned = True
      TabOrder = 4
      Width = 183
    end
  end
  object btnLuk: TcxButton
    Left = 735
    Top = 574
    Width = 105
    Height = 25
    Action = actLuk
    Anchors = [akRight, akBottom]
    LookAndFeel.NativeStyle = True
    OptionsImage.Glyph.Data = {
      
      D3D8DBDDE1EDED1515151515151515EEF2F2F2F2F11515151515}
    TabOrder = 1
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
  end
  object cxButton3: TcxButton
    Left = 311
    Top = 573
    Width = 153
    Height = 25
    Action = actSlet
    Anchors = [akBottom]
    TabOrder = 2
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
  end
  object cxButton2: TcxButton
    Left = 166
    Top = 573
    Width = 153
    Height = 25
    Action = actOpret
    Anchors = [akBottom]
    TabOrder = 3
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
  end
  object cxButton1: TcxButton
    Left = 27
    Top = 573
    Width = 153
    Height = 25
    Action = actRediger
    Anchors = [akLeft, akBottom]
    TabOrder = 4
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
  end
  object dsproducts: TDataSource
    Left = 560
    Top = 1
  end
  object ActionList1: TActionList
    Left = 488
    Top = 1
    object actRediger: TAction
      Caption = 'Rediger vare  (F3)'
      ShortCut = 114
      OnExecute = actRedigerExecute
    end
    object actOpret: TAction
      Caption = 'Opret vare (F2)'
      ShortCut = 113
      OnExecute = actOpretExecute
    end
    object actSlet: TAction
      Caption = 'Slet vare (F4)'
      ShortCut = 115
      OnExecute = actSletExecute
    end
    object actLuk: TAction
      Caption = 'Luk (F10)'
      ShortCut = 121
      OnExecute = actLukExecute
    end
    object actRens: TAction
      OnExecute = actRensExecute
    end
  end
end
